...
public static class Whisper
{
    ...
    private static InferenceSession InitializeModel()
    {
        ...
        try
        {
            string modelPath = $@"{AppDomain.CurrentDomain.BaseDirectory}onnx-models\whisper\whisper_medium_int8_cpu_ort_1.18.0.onnx";
    ...
    private static async Task<List<WhisperTranscribedChunk>> TranscribeChunkAsync(float[] pcmAudioData, string inputLanguage, WhisperTaskType taskType, int offsetSeconds = 30)
    {
        ...
        try
        {
            ...
            DenseTensor<float> audioTensor = new(pcmAudioData, [1, pcmAudioData.Length]);
            DenseTensor<int> timestampsEnableTensor = new(new[] { 1 }, [1]);
            ...
            int[] decoderInputIds = new int[] { 50258, langCode, task };
            DenseTensor<int> langAndModeTensor = new(decoderInputIds, [1, 3]);
            ...
            List<WhisperTranscribedChunk> transcribedChunks = WhisperUtils.ProcessTranscriptionWithTimestamps(result, offsetSeconds);
    ...
    public async static Task<List<WhisperTranscribedChunk>> TranscribeAsync(StorageFile audioFile, EventHandler<float>? progress = null)
    {
        ...
        List<WhisperTranscribedChunk> transcribedChunks = [];
        ...
        try
        {
            ...
            byte[]? audioBytes = Utils.LoadAudioBytes(audioFile.Path);
            ...
            List<WhisperChunk>? dynamicChunks = WhisperChunking.SmartChunking(audioBytes);
            ...
            for (int i = 0; i < dynamicChunks.Count; i++)
            {
                WhisperChunk chunk = dynamicChunks[i];
                ...
                try
                {
                    float[]? audioSegment = Utils.ExtractAudioSegment(audioFile.Path, chunk.Start, chunk.End - chunk.Start);
                    Debug.WriteLine($"[Whisper] Extracted audio segment: {audioSegment?.Length ?? 0} samples");

                    if (audioSegment != null && audioSegment.Length > 0)
                    {
                        List<WhisperTranscribedChunk> transcription = await TranscribeChunkAsync(audioSegment, "en", WhisperTaskType.Transcribe, (int)chunk.Start);
