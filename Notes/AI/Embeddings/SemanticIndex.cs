...
public partial class SemanticIndex
{
    ...
    public async Task<List<TextChunk>> Search(string searchTerm, int top = 5)
    {
        ...
        List<TextChunk> storedVectors = dataContext.TextChunks.Select(chunk => chunk).ToList();

        float[][] searchVectors = await Embeddings.Instance.GetEmbeddingsAsync(searchTerm).ConfigureAwait(false);
        int[] ranking = CalculateRanking(searchVectors.First(), [.. storedVectors.Select(chunk => chunk.Vectors)]);

        for (int i = 0; i < ranking.Length && i < top; i++)
        {
    ...
    public static int[] CalculateRanking(float[] searchVector, List<float[]> vectors)
    {
        ...
        {
            float score = TensorPrimitives.CosineSimilarity(vectors[i], searchVector);
            scores[i] = score;
        }
        ...
        indexranks = [.. indexedFloats.Select(item => item.Index)];
    ...
    private List<string> SplitParagraphInChunks(string paragraph, int maxLength)
    {
        ...
        string[] sentences = paragraph.Split(new[] { "." }, StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries);
    ...
    public async Task AddOrReplaceContent(string content, int sourceId, string contentType, EventHandler<float>? progress = null)
    {
        ...
        await Task.Run(async () =>
        {
            ...
            {
                List<TextChunk> chunkBatch = chunks.Skip(i).Take(chunkBatchSize).ToList();
                float[][] vectors = await Embeddings.Instance.GetEmbeddingsAsync([.. chunkBatch.Select(c => c.Text)]).ConfigureAwait(false);
